...
const categories = [
    ...
    { name: "Plomeria", icon: <Pipette size={28} />, path: "/productos?categoria=plomeria", color: "bg-cyan-100 text-cyan-600" },
...
const Categories = () => {
    return (
        <section className="max-w-[1400px] mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-8">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6">Comprar por Categoria</h2>
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-2 sm:gap-3">
                {categories.map((cat) => (
                    <Link
                        key={cat.name}
                        to={cat.path}
                        className="flex flex-col items-center gap-2 p-2 sm:p-4 rounded-xl sm:rounded-2xl hover:shadow-md transition group cursor-pointer bg-white border border-gray-100 min-w-0"
                    >
